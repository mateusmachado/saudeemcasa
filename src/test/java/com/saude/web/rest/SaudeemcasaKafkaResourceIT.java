...
import com.saude.config.KafkaProperties;
...
class SaudeemcasaKafkaResourceIT {
    ...
    void setup() {
        ...
        SaudeemcasaKafkaResource kafkaResource = new SaudeemcasaKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/saudeemcasa-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/saudeemcasa-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
